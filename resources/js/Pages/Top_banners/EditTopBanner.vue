<template>
    <Layout>
        ...
        <div class="p-6 space-y-8 rounded-lg">
            ...
            <form class="mt-8 space-y-6 from" @submit.prevent="send" action="/banner/edit" method="post" encType="multipart/form-data">
                <div class="mb-6">
...
import {topBannerInfo,topBannerEdit} from '@/api.js';
...
export default {
    ...
    methods: {
        send(id) {
            topBannerEdit(this.form, id).then(() => {
    ...
    mounted() {
        topBannerInfo(this.$route.params.id).then((res) => {
            this.form = res;
        });
    }
}
